...
if (isset($_POST['TambahSertifikat'])) {
    ...
    if ($stmt_insert->execute()) {
        $_SESSION['guru_sertif_success'] = "Sertifikat Berhasil Ditambahkan!";
        header("Location: datasertifikat.php");
        exit();
    } else {
        $_SESSION['guru_sertif_error'] = "Error: " . $koneksi->error;
        header("Location: datasertifikat.php");
        exit();
    }
}
...
if (isset($_POST['EditSertifikat'])) {
    ...
    if ($_FILES['file_sertifikat']['name'] != "") {
        ...
        if ($result) {
            $_SESSION['guru_sertif_success'] = "Data Sertifikat Berhasil Diubah!";
            header("Location: datasertifikat.php");
            exit();
        }
    } else {
...
if (isset($_GET['id_sertifikat'])) {
    ...
    if ($result) {
        $_SESSION['guru_sertif_success'] = "Data Sertifikat Berhasil Dihapus!";
        header("Location: datasertifikat.php");
        exit();
    }
}
            ...
            <main>
                <div class="container-fluid px-4">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <?php
                            if (isset($_SESSION['guru_sertif_error']) && !empty($_SESSION['guru_sertif_error'])) {
                                echo '<div class="alert alert-danger" role="alert">' . $_SESSION['guru_sertif_error'] . '</div>';
                                unset($_SESSION['guru_sertif_error']);
                            }

                            if (isset($_SESSION['guru_sertif_success']) && !empty($_SESSION['guru_sertif_success'])) {
                                echo '<div class="alert alert-success" role="alert">' . $_SESSION['guru_sertif_success'] . '</div>';
                                unset($_SESSION['guru_sertif_success']);
                            }
                            ?>
